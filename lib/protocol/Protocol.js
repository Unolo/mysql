...
Protocol.prototype._enqueue = function(sequence) {
  ...
  let shouldLog = sequence._connection && process.env.MYSQL_QUERY_LOGGING==1;// && sequence.sql.toLowerCase().startsWith('select');

  // MYSQL_SLOW_QUERY_MS set: skip the per-query start line and log a single
  // "query took N ms sql:..." line at completion, only for queries over the
  // threshold. Unset: legacy behavior (log every query start + every took).
  let slowOnlyMs = null;
  if (shouldLog && process.env.MYSQL_SLOW_QUERY_MS !== undefined){
    slowOnlyMs = parseFloat(process.env.MYSQL_SLOW_QUERY_MS);
    if (isNaN(slowOnlyMs)) slowOnlyMs = 200;
  }
  ...
  if (shouldLog){
    ...
    // Capture the ISSUER's async context now. The 'end' handler below fires from the
    // socket's data path, whose AsyncLocalStorage context is whatever request happened to
    // be active when this pooled connection was CREATED — possibly hours ago. Logging
    // there stamps that stale request's [ctx req=...] on every slow-query line this
    // connection ever emits (observed: one 618ms request tagged on 4,900+ SQL lines over
    // 5h, because its dataloader fan-out created 36 pool connections). runInAsyncScope()
    // at emit time re-enters the context captured here, so the winston formatter resolves
    // the request that actually issued the query.
    sequence._logCtx = new AsyncResource('mysql:slow-query-log');
    if (slowOnlyMs === null){
      let sql = sequence.sql;
      if (sql.length>200 && sql.indexOf('leavesv2')===-1){
        sql = sql.substring(0,200);
      }

      console.log("[%s] query with sql:%s",sequence._connection.threadId,sql.replace(/(\r\n|\n|\r)/gm, ""));
    }
  }
  ...
  sequence.on('end', function () {
    ...
    if (shouldLog){
      ...
      if (slowOnlyMs === null){
        sequence._logCtx.runInAsyncScope(function(){
          console.log("[%s] query took %s ms",sequence._connection.threadId,took);
        });
      } else if (took > slowOnlyMs){
        let sql = sequence.sql || '';
        if (sql.length>200 && sql.indexOf('leavesv2')===-1){
          sql = sql.substring(0,200);
        }
        sequence._logCtx.runInAsyncScope(function(){
          console.log("[%s] query took %s ms sql:%s",sequence._connection.threadId,took,sql.replace(/(\r\n|\n|\r)/gm, ""));
        });
      }
      sequence._logCtx.emitDestroy();
    }
