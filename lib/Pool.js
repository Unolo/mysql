...
Pool.prototype.query = function (sql, values, cb) {
  ...
  let shouldLog = process.env.MYSQL_QUERY_LOGGING==1 && sql.toLowerCase().startsWith('select');
  ...
  this.getConnection(function (err, conn) {
    ...
    query.once('end', function() {
      ...
      if (shouldLog){
        console.log("[%s] query took %s ms",conn.threadId,performance.now() - start);  
      }
    ...
    if (shouldLog){
      console.log("[%s] query with sql:%s",conn.threadId,sql.replace(/(\r\n|\n|\r)/gm, ""));      
    }
