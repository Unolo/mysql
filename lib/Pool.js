...
const CONNECTION_HOLD_TIMEOUT = 540000; // 9 minutes — deliberately < the 600s (10min) server wait_timeout so the leak-detection stack trace is logged before the connection is reaped
...
function handleTimeout(threadId, connection) {
  var pool = connection._pool;

  // The connection may already have been released/removed from the pool by the
  // time this timer fires (e.g. it was purged on a connect/ping failure). In
  // that case _pool is null and there is nothing left to clean up — bail out
  // instead of throwing an uncaught exception.
  if (!pool) {
    return;
  }

  // LOG ONLY — deliberately do NOT rollback/destroy here. A connection held past
  // the timeout may be legitimately doing long work (a big query, a batch job), and
  // rolling it back / destroying it mid-operation would corrupt live work. The job
  // of this timer is diagnostic: surface WHERE a long-held (potentially leaked)
  // connection was acquired, so the leaking code path can be found and fixed.
  // Reclaiming a genuinely-leaked (idle) connection is handled safely server-side by
  // the per-session wait_timeout, which only reaps idle sessions — never an actively
  // running query.
  console.error(`🚨 Connection held >9 minutes (possible leak) for connectionID:${threadId}`);
  ...
  // Clear our tracking for this connection. Use the threadId captured when the timer
  // was armed (handleGetConnection keys _timeouts/_connectionMetadata by it), not
  // connection.threadId, which may have changed if the connection reconnected.
  delete pool._timeouts[threadId];
  delete pool._connectionMetadata[threadId];
}
...
Pool.prototype._removeConnection = function(connection) {
  // Clear any pending leak-detection timer before detaching the pool, otherwise
  // it would fire later with connection._pool === null and crash the process.
  if (this._timeouts[connection.threadId]) {
    clearTimeout(this._timeouts[connection.threadId]);
    delete this._timeouts[connection.threadId];
    delete this._connectionMetadata[connection.threadId];
  }
